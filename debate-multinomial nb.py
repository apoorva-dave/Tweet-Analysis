...
# TODO : preprocessing to be improved.
...
def read_data():
    data = pd.read_csv("C:/Users/Apoorva/PycharmProjects/TweetsClassification/Sentiment.csv", header=0, delimiter=",", quoting=15)
...
def randomize(data):
    data["Sentiment"] = data["sentiment"].apply(lambda sentiment: "1" if sentiment == "Positive" else "-1" if sentiment == "Negative" else "0")
...
def train_data(sentiment_data):
    #return random_data
    # 80% for training
    train_X, train_y = zip(*sentiment_data[:13000])
...
def test_data(sentiment_data):
    # Keep 20% for testin
    test_X, test_y = zip(*sentiment_data[13001:])
...
#
    ...
    # unigram_bigram_clf = Pipeline([
    #     ('vectorizer', TfidfVectorizer(ngram_range=(1, 2), tokenizer=stemming_tokenizer,
    #                                    stop_words=stopwords.words('english') + list(string.punctuation))),
    #     ('classifier', MultinomialNB(alpha=0.05))
    # ])
...
def main():
    ...
    # print(test_y[0])
    #print(random_data)
    unigrams(train_X,train_y,test_X,test_y)     #0.703
    bigrams(train_X,train_y,test_X,test_y)      #0.706
    unigram_bigram(train_X,train_y,test_X,test_y)  #0.715
